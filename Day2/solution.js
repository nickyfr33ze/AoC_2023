// Code written by @nickyfr33ze on Dec 8, 2023
...
game 3 would have been impossible because at one point the Elf showed you 20 red cubes at once; similarly, game 4 would also have been impossible 
...
// Function to parse the input into game objects
const parseInput = (input) => {
  const games = [];
  for (let i = 0; i < input.length; i++) {
    const game = {};
    const id = input[i].split(':')[0].split(' ')[1];
    game.id = id;
...
// Parsing the input into game objects
const games = parseInput(input);

// Function to check if a game is possible with given colors
const checkGame = (game, colors) => {
    const bag = { red: 0, green: 0, blue: 0 };
    for (let i = 0; i < game.sets.length; i++) {
        const set = game.sets[i].trim().split(' ');
        const color = set[1];
        const amount = set[0];
...
// Function to check all games and return possible game IDs
const checkGames = (games, colors) => {
    const possibleGames = [];
    for (let i = 0; i < games.length; i++) {
        if (checkGame(games[i], colors)) {
        possibleGames.push(games[i].id);
        }
    }
    return possibleGames;
}

// Checking all games and getting possible game IDs
const possibleGames = checkGames(games, colors);

// Function to calculate the sum of game IDs
const sumGames = (games) => {
    let sum = 0;
    for (let i = 0; i < games.length; i++) {
        sum += parseInt(games[i]);
    }
    return sum;
}

// Calculating the sum of game IDs and printing the result
const sum = sumGames(possibleGames);
